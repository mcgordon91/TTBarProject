...
for sample in dictOfListOfFiles:
    ...
    rdfTriggersAndMasks[sample] = rdf[sample].Filter("HLT_IsoMu27 == true", "HLTTriggerSingleMuonTrigger")        .Define("LumiXS",wgtFormula[sample])        .Define("isolated_mu_mask", "Muon_pt > 30 && abs(Muon_eta) < 2.4 && Muon_tightId == true && Muon_pfIsoId >= 4")        .Define("isolated_mu_veto", "Muon_pt > 10 && abs(Muon_eta) < 2.4 && Muon_looseId == true && Muon_pfIsoId >= 4 && isolated_mu_mask == false")        .Define("jpsi_mu_candidate_mask", "Muon_pt > 3 && Muon_mediumId == true && isolated_mu_mask == false")
    ...
    rdfOnePlusMuons[sample] = rdfTriggersAndMasks[sample].Filter("Sum(isolated_mu_mask) == 1", "Exactly One Good Isolated Muon")        .Define("IsolatedMuon_pt", "Muon_pt[isolated_mu_mask]")        .Define("IsolatedMuon_eta", "Muon_eta[isolated_mu_mask]")        .Define("IsolatedMuon_phi", "Muon_phi[isolated_mu_mask]")        .Define("IsolatedMuon_mass", "Muon_mass[isolated_mu_mask]")        .Define("IsolatedMuon_charge", "Muon_charge[isolated_mu_mask]")        .Define("IsolatedMuon_pfRelIso03_all", "Muon_pfRelIso03_all[isolated_mu_mask]")        .Define("IsolatedMuon_pfRelIso03_chg", "Muon_pfRelIso03_chg[isolated_mu_mask]")        .Define("IsolatedMuon_pfRelIso04_all", "Muon_pfRelIso04_all[isolated_mu_mask]")
    # No filter here, but criteria for jets are spelled out and the pt, eta, phi, etc. of jets are defined
    rdfJetVariables[sample] = rdfOnePlusMuons[sample]        .Define("jet_mask", "ROOT::VecOps::RVec<Int_t> jmask = (Jet_pt >= 30 && abs(Jet_eta) <= 2.5 && Jet_jetId >= 2); "                          "for(int i=0; i < IsolatedMuon_pt.size(); ++i){"                              "ROOT::VecOps::RVec<Float_t> dr;"                              "for(int j=0; j < jmask.size(); ++j){"                                  "dr.push_back(ROOT::VecOps::DeltaR(Jet_eta.at(j), IsolatedMuon_eta.at(i), Jet_phi.at(j), IsolatedMuon_phi.at(i)));}"                                  "jmask = jmask && dr >= 0.4;"                                  "dr.clear();}"                          "return jmask;")        .Define("MediumBJetMask", "Jet_btagDeepFlavB > 0.3033 && jet_mask" )        .Define("MTofMETandMu", "FTA::transverseMassMET(IsolatedMuon_pt, IsolatedMuon_phi, IsolatedMuon_mass, MET_pt, MET_phi)")        .Define("Num_Jets", "Jet_pt[jet_mask].size()")        .Define("SJet1_pt", "Jet_pt[jet_mask].size() > 0 ? Jet_pt[jet_mask].at(0) : -500")        .Define("SJet2_pt", "Jet_pt[jet_mask].size() > 1 ? Jet_pt[jet_mask].at(1) : -500")        .Define("SJet1_eta", "Jet_eta[jet_mask].size() > 0 ? Jet_eta[jet_mask].at(0) : 500")        .Define("SJet2_eta", "Jet_eta[jet_mask].size() > 1 ? Jet_eta[jet_mask].at(1) : 500")        .Define("SJet1_phi", "Jet_phi[jet_mask].size() > 0 ? Jet_phi[jet_mask].at(0) : 500")        .Define("SJet2_phi", "Jet_phi[jet_mask].size() > 1 ? Jet_phi[jet_mask].at(1) : 500")        .Define("SJet_btagDeepFlavB", "Jet_btagDeepFlavB[jet_mask]")        .Define("Num_BTaggedJets", "Sum(MediumBJetMask)")        .Define("Ht", "Sum(Jet_pt[jet_mask])")
    # This third filter leaves only events with at least 4 jets
    rdfTwoPlusJets[sample] = rdfJetVariables[sample].Filter("Num_Jets >= 4", "At Least Four Jets")        .Define("DeepJetB", "Jet_pt[jet_mask].size() > 0 ? Jet_btagDeepFlavB[jet_mask].at(0) : 0")        .Define("Num_Muons", "Muon_pt[isolated_mu_mask].size()")
    ...
    rdfOtherMuons[sample] = rdfmu[sample].Filter("Sum(jpsi_mu_candidate_mask) >= 2", "JPsi Candidate")        .Define("JPsiMuons_pt", "Muon_pt[jpsi_mu_candidate_mask]")        .Define("JPsiMuons_eta", "Muon_eta[jpsi_mu_candidate_mask]")        .Define("JPsiMuons_phi", "Muon_phi[jpsi_mu_candidate_mask]")        .Define("JPsiMuons_charge", "Muon_charge[jpsi_mu_candidate_mask]")        .Define("JPsiMuons_mass", "Muon_mass[jpsi_mu_candidate_mask]")        .Define("JPsiMuons_multiplicity", "Muon_pt[jpsi_mu_candidate_mask].size()")        .Define("InvariantMasses", "std::cout << rdfentry_ << std::endl; int FlagPair = -1; auto c = MatchOppositelyChargedMuons(JPsiMuons_pt, JPsiMuons_eta, JPsiMuons_phi, JPsiMuons_mass, JPsiMuons_charge, event, FlagPair); return c.OppositelyChargedMuonInvariantMassCalculator()")        .Define("InvariantMassesAllMuons", "std::cout << rdfentry_ << std::endl; int FlagPair = -1; auto c = MatchOppositelyChargedMuons(IsolatedMuon_pt, IsolatedMuon_eta, IsolatedMuon_phi, IsolatedMuon_mass, IsolatedMuon_charge, event, FlagPair, JPsiMuons_pt, JPsiMuons_eta, JPsiMuons_phi, JPsiMuons_mass, JPsiMuons_charge); return c.ThreeMuonInvariantMassCalculator()")        .Define("DeltaEta", "int FlagPair = -1; auto c = MatchOppositelyChargedMuons(IsolatedMuon_pt, IsolatedMuon_eta, IsolatedMuon_phi, IsolatedMuon_mass, IsolatedMuon_charge, event, FlagPair, JPsiMuons_pt, JPsiMuons_eta, JPsiMuons_phi, JPsiMuons_mass, JPsiMuons_charge); return c.DeltaEtaCalculator()")        .Define("DeltaPhi", "int FlagPair = -1; auto c = MatchOppositelyChargedMuons(IsolatedMuon_pt, IsolatedMuon_eta, IsolatedMuon_phi, IsolatedMuon_mass, IsolatedMuon_charge, event, FlagPair, JPsiMuons_pt, JPsiMuons_eta, JPsiMuons_phi, JPsiMuons_mass, JPsiMuons_charge); return c.DeltaPhiCalculator()")        .Define("DeltaR", "int FlagPair = -1; auto c = MatchOppositelyChargedMuons(IsolatedMuon_pt, IsolatedMuon_eta, IsolatedMuon_phi, IsolatedMuon_mass, IsolatedMuon_charge, event, FlagPair, JPsiMuons_pt, JPsiMuons_eta, JPsiMuons_phi, JPsiMuons_mass, JPsiMuons_charge); return c.DeltaRCalculator()")
