...
JPsiMuonInPeakMask = "ROOT::VecOps::RVec<Int_t> jpsipeakmask = (jpsi_mu_candidate_mask);"                            "RVec_f pt {};"                            "RVec_f eta {};"                            "RVec_f phi {};"                            "RVec_f mass {};"                            "for(int i = 0; i < JPsiCandidate_pt.size(); i++)"                                "{"                                    "ROOT::VecOps::RVec<Int_t> charges;"                                    "ROOT::VecOps::RVec<Float_t> im;"                                    "for(int j = 0; j < jpsipeakmask.size(); j++)"                                    "{"                                        "charges.push_back(JPsiCandidate_charge[i] + JPsiCandidate_charge[j]);"                                        "pt.push_back(JPsiCandidate_pt[i]);"                                        "eta.push_back(JPsiCandidate_eta[i]);"                                        "phi.push_back(JPsiCandidate_phi[i]);"                                        "mass.push_back(JPsiCandidate_mass[i]);"                                        "pt.push_back(JPsiCandidate_pt[j]);"                                        "eta.push_back(JPsiCandidate_eta[j]);"                                        "phi.push_back(JPsiCandidate_phi[j]);"                                        "mass.push_back(JPsiCandidate_mass[j]);"                                        "im.push_back(ROOT::VecOps::InvariantMass(pt, eta, phi, mass));"                                        "pt.clear();"                                        "eta.clear();"                                        "phi.clear();"                                        "mass.clear();"                                    "}"                                    "jpsipeakmask = charges == 0 && im >= 3.0 && im <= 3.2;"                                    "charges.clear();"                                    "im.clear();"                                "}"                            "return jpsipeakmask;"
...
RVec_FourVector MuonAndJPsiStatisticsAndKinematics::JPsiForMuonsInPeakFourVectorCalculator()
{
    ...
    {    
        ...
        for(int j = i+1; j < this->JPsi_Muon_charge.size(); j++)
        {
            ...
            if((FirstMuonCharge * SecondMuonCharge == -1) && (ROOT::VecOps::InvariantMass(pt, eta, phi, mass) > 3.0) && (ROOT::VecOps::InvariantMass(pt, eta, phi, mass) < high_jpsi_im_cut))
            {
                auto FourVector1 = ROOT::Math::PtEtaPhiMVector (this->JPsi_Muon_pt[i], this->JPsi_Muon_eta[i], this->JPsi_Muon_phi[i], this->JPsi_Muon_mass[i]);
    ...
    for(int k = 0; k < this->Isolated_Muon_pt.size(); k++)
    {
        ...
        {    
            ...
            for(int j = i+1; j < this->JPsi_Muon_charge.size(); j++)
            {
                ...
                if((FirstMuonCharge * SecondMuonCharge == -1) && (ROOT::VecOps::InvariantMass(pt, eta, phi, mass) > low_jpsi_im_cut) && (ROOT::VecOps::InvariantMass(pt, eta, phi, mass) < 3.2))
                {
                    auto FourVector1 = ROOT::Math::PtEtaPhiMVector (this->JPsi_Muon_pt[i], this->JPsi_Muon_eta[i], this->JPsi_Muon_phi[i], this->JPsi_Muon_mass[i]);
...
for sample in dictOfListOfFiles:
    ...
    if sample == 'Data':
        ...
        hist['Data']["ht"] = rdfJetAndIsolatedLeptonFiltered['Data'].Histo1D(("ht", "Ht; Ht; Events", 75, 0, 1500), "Ht")
        
        hist['Data']["met"] = rdfJPsiMuons['Data'].Histo1D(("met", "MET; Pt (GeV); Events",100,0,250),"METAfterMETCut")
        ...
        hist['Data']["invariant_mass_jpsi_muons_and_isolated_muons"] = rdfJPsiMuons['Data'].Histo1D(("invariant_mass_jpsi_and_isolated_muons", "Invariant Masses for J/Psi And Isolated Muons (dR < 2); Invariant Masses; Events", 100, 0, 200), "InvariantMassJPsiMuonsAndIsolatedMuons")
        ...
        hist['Data']["top_mass_in_peak"] = rdfJPsiMuons['Data'].Histo1D(("top_mass_in_peak", "Isolated and JPsi Muons Mass; Mass; Events", 60, 0, 300), "TopMassInPeak")
        ...
        hist[sample]["ht"] = rdfJPsiMuons[sample].Histo1D((sample + "_" + "ht", "Monte Carlo " + sample + "; Ht; Ht; Events", 75, 0, 1500), "Ht", "LumiXS")
        
        hist[sample]["met"] = rdfJPsiMuons[sample].Histo1D((sample + "_" + "met", "Monte Carlo " + sample + "; MET; Pt (GeV); Events",100,0,250), "METAfterMETCut", "LumiXS")
        ...
        hist[sample]["top_mass_in_peak"] = rdfJPsiMuons[sample].Histo1D((sample + "_" + "top_mass_in_peak", "Monte Carlo" + sample + "; Isolated and JPsi Muons Mass; Mass; Events", 60, 0, 300), "TopMassInPeak", "LumiXS")
